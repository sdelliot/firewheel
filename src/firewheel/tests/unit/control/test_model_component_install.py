...
from firewheel.control.model_component_install import (
...
def test_install_mc_success(mock_chmod, mock_stat, mock_open, install_component):
    ...
    mock_open.return_value.__enter__.return_value = (
        mock_file  # Simulate the context manager
    )
...
)  # install_script exists, install_flag does not exist
@patch(
    "firewheel.control.model_component_install.InstallPrompt.ask", return_value="y"
)  # Simulate user input
@patch(
    "firewheel.control.model_component_install.ModelComponentInstall.install_mc",
    return_value=False,
)  # Mock install_mc to return True
...
def test_run_ansible_playbook_success(
    ...
    assert (
        result is True
    )  # Expecting True since the playbook should execute successfully


@patch("firewheel.control.model_component_install.ansible_runner.RunnerConfig")
@patch("firewheel.control.model_component_install.Path.open", new_callable=MagicMock)
@patch(
    "firewheel.control.model_component_install.config", new_callable=MagicMock
)  # Mock the config dictionary
def test_run_ansible_playbook_with_cache(
